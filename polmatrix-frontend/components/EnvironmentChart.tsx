...
import { Share2, Download, BarChart2, LineChart as LineChartIcon, AreaChart as AreaChartIcon, Check, Info, Leaf } from 'lucide-react'
...
const downloadCSV = (data: Props["data"], metrics: string[], filename = "environment_data.csv") => {
...
export default function EnvironmentChart({ data, selectedMetrics, onToggleMetric }: Props) {
  ...
  const defaultMetrics = ["co2_emissions", "renewable_energy_percentage", "forest_area_percentage", "air_pollution_index"];
  ...
  const handleShare = useCallback(() => {
    ...
    if (navigator.share) {
      navigator.share({
        title: 'Environment Metrics Data',
    ...
    <div className="p-6 rounded-xl shadow bg-[var(--card-bg)] transition-transform transform hover:scale-[1.005] hover:shadow-2xl hover:cursor-pointer relative hover:border hover:border-[var(--turquoise)] hover:opacity-100 opacity-90">
        ...
        <div>
          ...
          {effectiveMetrics.length === 0 && (
            <p className="text-sm mt-1 text-[var(--text-secondary)]">
              Select metrics from the sidebar to visualize environmental trends
            </p>
          )}
        </div>
      ...
      {effectiveMetrics.length === 0 ? (
        <div className="h-[300px] flex items-center justify-center flex-col text-[var(--text-secondary)] bg-[var(--input-bg)] rounded-lg border border-dashed border-[var(--border-color)]">
          <Leaf size={40} className="opacity-40 mb-3 text-green-500" />
          <p className="font-medium">Select metrics to visualize environmental data</p>
