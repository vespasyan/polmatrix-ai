...
import { Share2, Download, BarChart2, LineChart as LineChartIcon, AreaChart as AreaChartIcon, Check, Info, Heart } from 'lucide-react'
...
const metricMap: Record<string, { label: string; color: string; description: string; unit: string }> = Object.fromEntries(
  metricsConfig.health.map((metric, index) => [
    metric.key,
    {
      label: metric.label,
    ...
    return (
      <div className="p-3 bg-[var(--tooltip-bg)] border border-[var(--border-color)] rounded-lg shadow-lg text-sm text-[var(--text-secondary)] animate-in fade-in duration-200 max-w-[250px]">
        ...
        {payload.map((entry) => {
          ...
          const config = metricKey ? metricMap[metricKey] : null;
          
          return (
            <div key={entry.name} className="mb-1.5">
              <p className="flex items-center gap-1.5" style={{ color: entry.color }}>
                <span className="font-medium">{`${entry.name}:`}</span>
                <span className="font-bold">{entry.value}{config?.unit && ` ${config.unit}`}</span>
              </p>
              {config?.description && (
                <p className="text-xs mt-0.5 opacity-80 pl-1">{config.description}</p>
              )}
            </div>
          );
        })}
...
const downloadCSV = (data: Props["data"], metrics: string[], filename = "health_data.csv") => {
...
export default function HealthChart({ data, selectedMetrics, onToggleMetric }: Props) {
  ...
  const defaultMetrics = ["life_expectancy", "maternal_mortality_rate", "healthcare_expenditure_per_capita", "physician_density"];
  ...
  const renderChart = useCallback(() => {
    ...
    const commonElements = (
      <>
        ...
        />
        <Legend
          onClick={(e: any) => onToggleMetric(e.dataKey)}
          wrapperStyle={{ 
            cursor: 'pointer', 
            userSelect: 'none',
            padding: '10px 0 0',
    ...
    <div className="p-6 rounded-xl shadow bg-[var(--card-bg)] transition-transform transform hover:scale-[1.005] hover:shadow-2xl hover:cursor-pointer relative hover:border hover:border-[var(--turquoise)] hover:opacity-100 opacity-90">
        ...
        <div>
          ...
          {effectiveMetrics.length === 0 && (
            <p className="text-sm mt-1 text-[var(--text-secondary)]">
              Select metrics from the sidebar to visualize health trends
            </p>
          )}
        </div>
      ...
      {effectiveMetrics.length === 0 ? (
        <div className="h-[300px] flex items-center justify-center flex-col text-[var(--text-secondary)] bg-[var(--input-bg)] rounded-lg border border-dashed border-[var(--border-color)]">
          <Heart size={40} className="opacity-40 mb-3 text-red-500" />
          <p className="font-medium">Select metrics to visualize health data</p>
