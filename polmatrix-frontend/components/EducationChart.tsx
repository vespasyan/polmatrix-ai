...
const generateColor = (index: number) => {
  const colors = [
    "#3B82F6", "#22C55E", "#F97316", "#8B5CF6", "#EF4444", "#06B6D4", "#84CC16", "#F59E0B"
  ]
...
const downloadCSV = (data: Props["data"], metrics: string[], filename = "education_data.csv") => {
...
export default function EducationChart({ data, selectedMetrics, onToggleMetric }: Props) {
  ...
  const defaultMetrics = ["literacy_rate", "school_enrollment_rate", "education_expenditure", "primary_completion_rate"];
    ...
    <div className="p-6 rounded-xl shadow bg-[var(--card-bg)] transition-transform transform hover:scale-[1.005] hover:shadow-2xl hover:cursor-pointer relative hover:border hover:border-[var(--turquoise)] hover:opacity-100 opacity-90">
        ...
        <div>
          ...
          {effectiveMetrics.length === 0 && (
            <p className="text-sm mt-1 text-[var(--text-secondary)]">
              Select metrics from the sidebar to visualize education trends
            </p>
          )}
        </div>
      ...
      {effectiveMetrics.length === 0 ? (
        <div className="h-[300px] flex items-center justify-center flex-col text-[var(--text-secondary)] bg-[var(--input-bg)] rounded-lg border border-dashed border-[var(--border-color)]">
          ...
          <p className="font-medium">Select metrics to visualize education data</p>
